...
static bool sdrv_mbox_startup(mbox_chan_t *chan, void *priv_data);
...
static bool sdrv_mbox_tx_done(mbox_chan_t *chan, uint32 timeout);
...
#else
static void sdrv_mbox_irq_handler(void)
#endif
{
#if defined(NON_AUTOSAR_STANDARD_IRQ_HANDLER)
    mbox_dev_t *dev = arg;
