...
/**
 * @brief dmac mux direction
 */
typedef enum {
    DMA_MUX_RD = 0x1, /* RD PERi to MEM -> RX */
 ...
 * @brief dmac mux port
 */
typedef struct sdrv_dma_mux_param {
    uint32 port;
    uint64 start_addr;
    uint64 len;
    DMA_MUX_DIR direct;
...
/**
 * @brief dmac transfer addr capability
 */
typedef struct sdrv_dma_addr_range {
    uint64 base_addr;
    uint64 len;
} sdrv_dma_addr_range_t;

/**
 * @brief dmac hardware config
 */
typedef struct sdrv_dma_cfg {
    sdrv_dma_addr_range_t *addr_table;
    uint32 addr_table_len;
    sdrv_dma_mux_param_t *mux_table;
    uint32 mux_table_len;
    const uint32 *irq_id_table;
    uint32 irq_num;
} sdrv_dma_cfg_t;
 ...
 *
 */
static uint32 sdrv_dma_crc32_table[256] = {
 ...
 */
static inline uint32 sdrv_dma_crc32(uint32 crc, const sint8 *buf,
                                    uint32 len)
{
    if (buf == NULL)
        return 0;
...
/**
 * @brief get channel transfor data with bytes
 *
