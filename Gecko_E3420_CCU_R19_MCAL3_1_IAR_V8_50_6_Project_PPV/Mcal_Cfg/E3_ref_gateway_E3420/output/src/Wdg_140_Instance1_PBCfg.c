/********************************************************************************************************
 * SEMIDRIVE Copyright Statement                                                                        *
 * Copyright (c) SEMIDRIVE. All rights reserved                                                         *
 *                                                                                                      *
 * This software and all rights therein are owned by SEMIDRIVE, and are                                 *
 * protected by copyright law and other relevant laws, regulations and                                  *
 * protection. Without SEMIDRIVE's prior written consent and/or related rights,                         *
 * please do not use this software or any potion thereof in any form or by any                          *
 * means. You may not reproduce, modify or distribute this software except in                           *
 * compliance with the License. Unless required by applicable law or agreed to                          *
 * in writing, software distributed under the License is distributed on                                 *
 * an 'AS IS' basis, WITHOUT WARRANTIES OF ANY KIND, either express or implied.                         *
 *                                                                                                      *
 *******************************************************************************************************/


/** *****************************************************************************************************
 *  \file     Wdg_140_Instance1_PBCfg.c                                                                 *
 *  \brief    This file contains interface header for WDG1 MCAL driver, ...                             *
 *                                                                                                      *
 * <table>                                                                                              *
 * <tr><th>Date           <th>Version                                                                   *
 * <tr><td>2023/07/17     <td>1.0.0                                                                     *
 * </table>                                                                                             *
 *******************************************************************************************************/
/********************************************************************************************************
 *                                      Include header files                                            *
 *******************************************************************************************************/
#include "Wdg_Cfg.h"
#include "Wdg_140_Instance1_Cfg.h"
#ifdef __cplusplus
extern "C" {
#endif
/********************************************************************************************************
 *                                 Global Macro definition                                              *
 *******************************************************************************************************/

/**
 *  \name WDG Driver ID Info
 *  @{
 */
/** \brief SemiDriver Instruments Vendor ID */
#define WDG_INSTANCE1_PBCFG_C_VENDOR_ID    0x8C
/* @} */
/**
 *  \name WDG Driver Module AUTOSAR Version Info
 *
 *  Defines for WDG Driver AUTOSAR version used for compatibility checks
 *  @{
 */
/** \brief AUTOSAR Major version specification implemented by WDG Driver */
#define WDG_INSTANCE1_PBCFG_C_AR_MAJOR_VERSION    4
/** \brief AUTOSAR Minor  version specification implemented by WDG Driver */
#define WDG_INSTANCE1_PBCFG_C_AR_MINOR_VERSION    3
/** \brief AUTOSAR Patch version specification implemented by WDG Driver */
#define WDG_INSTANCE1_PBCFG_C_AR_REVISION_VERSION 1
/* @} */
/**
 *  \name WDG Driver Module SW Version Info
 *
 *  Defines for WDG Driver version used for compatibility checks
 *  @{
 */
/** \brief Driver Implementation Major Version */
#define WDG_INSTANCE1_PBCFG_C_SW_MAJOR_VERSION    1
/** \brief Driver Implementation Minor Version */
#define WDG_INSTANCE1_PBCFG_C_SW_MINOR_VERSION    0
/** \brief Driver Implementation Patch Version */
#define WDG_INSTANCE1_PBCFG_C_SW_PATCH_VERSION    0
/* @} */
#if (WDG_INSTANCE1_SUPPORT == STD_ON)
/********************************************************************************************************
 *                                  Global Constant Declarations                                        *
 *******************************************************************************************************/

#define WDG1_START_SEC_CONST_UNSPECIFIED
#include "Wdg1_MemMap.h"
static const Wdg_ModeConfigType ModeSettings1[3] =
{
    /*Off mode*/
    {
        .clockFrequency   = 0U,
        .timeoutPeriod    = 0U,
        .timeoutBehavior  = WDG_INTERRUPT,
        .triggerCondition = NO_CONDITION_MODE,
        .refreshTime      = 0U
    },
    /*Slow mode*/
    {
        .clockFrequency   = 24000000U,
        .timeoutPeriod    = 50U,
        .timeoutBehavior  = WDG_RESET,
        .triggerCondition = NO_CONDITION_MODE,
        .refreshTime      = 0U,
        .externalSignalEnable = FALSE,
    },
    /*Fast mode*/
    {
        .clockFrequency   = 24000000U,
        .timeoutPeriod    = 50U,
        .timeoutBehavior  = WDG_RESET,
        .triggerCondition = NO_CONDITION_MODE,
        .refreshTime      = 0U,
        .externalSignalEnable = FALSE,
    },
};

const Wdg_ConfigType Wdg_Config1 =
{

    .initTimeout = 100U,
    .maxTimeout  = 1000U,
    .defaultMode = WDGIF_OFF_MODE,
    .currentInstance = WDT1_INDEX,
    .pfNotification = NULL_PTR,
    .modeSettings = ModeSettings1,
};
#define WDG1_STOP_SEC_CONST_UNSPECIFIED
#include "Wdg1_MemMap.h"
#endif /*#if (WDG_INSTANCE1_SUPPORT == STD_ON)*/
#ifdef __cplusplus
}
#endif
