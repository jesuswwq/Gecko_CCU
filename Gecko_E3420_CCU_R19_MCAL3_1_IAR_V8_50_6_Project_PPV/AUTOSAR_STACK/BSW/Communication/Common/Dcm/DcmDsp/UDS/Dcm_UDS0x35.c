...
static FUNC(Std_ReturnType, DCM_CODE) Dcm_0x35MemorySessionCheck(uint8 MemoryIdInfoIndex, uint8 MemoryRangeInfoIndex)
{
    uint8 index;
...
#define DCM_START_SEC_CODE
#include "Dcm_MemMap.h"
static FUNC(Std_ReturnType, DCM_CODE) Dcm_0x35MemorySecurityCheck(uint8 MemoryIdInfoIndex, uint8 MemoryRangeInfoIndex)
{
    uint8 index;
    Std_ReturnType ret = E_NOT_OK;
    P2CONST(Dcm_DspReadMemoryRangeInfoType, AUTOMATIC, DCM_VAR_NOINIT)
    ...
    {
        for (index = 0; (index < pDcmDspReadMemoryRangeInfo->DcmDspReadMemorySecurityLevelRefNum) && (E_NOT_OK == ret);
             index++)
        {
...
#endif
    /*min-length check*/
    if ((E_OK == ret) && (DCM_UDS0X35_REQ_DATA_MINLENGTH > Dcm_MsgCtrl[MsgCtrlId].MsgContext.ReqDataLen))
    {
