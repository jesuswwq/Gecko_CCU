 ...
 *  V2.0.1       2023-05-26  tong.zhao       fix Bug CPT-90
 *    Modify the bug of API Xcp_GetSeedHal when GET_SEED command to request
...
#if (0 != (XCP_PL_CAL & XCP_RESOURCE))
    calflag = Xcp_CheckAddress(Xcp_Mta2Ptr(extAddr, Addr), ((uint32)len * XCP_AG), XCP_MEM_CAL_ALL);
#endif
#if (0 != (XCP_PL_PGM & XCP_RESOURCE))
    pgmflag = Xcp_CheckAddress(Xcp_Mta2Ptr(extAddr, Addr), ((uint32)len * XCP_AG), XCP_MEM_PGM);
#endif
...
#endif
/*Check Mea address.*/
#if (0 != (XCP_PL_DAQ & XCP_RESOURCE))
    || ((Xcp_MeaStartAddr <= Addr ) & (Addr <= (Xcp_MeaStartAddr + Xcp_MeaLen)))
#endif
#if (0 != (XCP_PL_PGM & XCP_RESOURCE))
        || ((boolean)TRUE == pgmflag)
#endif
