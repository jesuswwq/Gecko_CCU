...
struct vic_data {
    ...
    struct sw_mask {
        ...
        struct pri_mask {
            ...
            */
            boolean masked[MAX_PRI + 1] ALIGNED(CACHE_LINE);
...
#define IRQM_START_SEC_VAR_INIT_UNSPECIFIED
#include "IrqM_MemMap.h"
...
#define IRQM_STOP_SEC_VAR_INIT_UNSPECIFIED
#include "IrqM_MemMap.h"
...
uint32  sdrv_vic_lld_mask_low_priority(uint32  pri_threshold, boolean user)
{
    ...
    ret = vicdata->old_mask;
    pri_threshold = MIN(pri_threshold, MAX_PRI + 1);

    if (user)
        vicdata->old_mask = pri_threshold;
...
void sdrv_vic_lld_init(uint32  base, uint32  intr_num, int (*isr)(uint32 ))
{
    ...
    g_vic_data[core_id].old_mask = ~0U;
...
#if (VIC_SOFT_IRQ_CLR_BY_HANDLER == 1)
    g_vic_data[core_id].isSoftIrq[vector] = 1;
#endif
    arch_irq_restore(state);
}
...
#if (VIC_SOFT_IRQ_CLR_BY_HANDLER == 1)
    g_vic_data[core_id].isSoftIrq[vector] = 0;
#endif
    arch_irq_restore(state);
}
...
{
    int core_id = get_core_id();
    return g_vic_data[core_id].isSoftIrq[vector];
}
#endif
