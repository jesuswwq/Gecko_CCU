...
void Mcu_SmcSocLp2wkupTimeoutConfig(uint32 base, uint16 tout_val, boolean tout_wkup_en)
{
    ...
    /* wdt disable between lp to wakeup event receive */
...
Std_ReturnType Mcu_SmcCoreIrqMonitor(uint32 base, uint32 core_idx, uint32 *irq_state)
{
    ...
    if ((core_idx < 5) && (irq_state != NULL_PTR)) {
...
Std_ReturnType Mcu_SmcComIrqMonitor(uint32 base, uint32 irq_grp_idx, uint32 *irq_state)
{
    ...
    if ((irq_grp_idx < 8) && (irq_state != NULL_PTR)) {
        if (irq_grp_idx == 0) {
...
{
    /* configure 24MHz，32KHz div number */
    Mcu_SmcSoc32kDivNumConfig(base, 0x1FU);
    Mcu_SmcSoc24mDivNumConfig(base, 0xFU);

    /* configure saf smc timeout */
    Mcu_SmcSafTimeoutConfig(base, TRUE, CKGEN_HK, 0xFFU);
    Mcu_SmcSafTimeoutConfig(base, TRUE, RSTGEN_HK, 0xFFU);
    Mcu_SmcSafTimeoutConfig(base, TRUE, MODE_REQ_TRANS, 0xFFU);
    Mcu_SmcSafTimeoutWkupConfig(base, TRUE);

#ifndef SEMIDRIVE_E3_LITE_SERIES
    /* configure ap smc timeout */
    Mcu_SmcApTimeoutConfig(base, TRUE, CKGEN_HK, 0xFFU);
    Mcu_SmcApTimeoutConfig(base, TRUE, RSTGEN_HK, 0xFFU);
    Mcu_SmcApTimeoutConfig(base, TRUE, MODE_REQ_TRANS, 0xFFU);
    Mcu_SmcApTimeoutWkupConfig(base, TRUE);
#endif

    /* configure soc swm smc timeout */
    Mcu_SmcSocSwmTimeoutConfig(base, TRUE, 0xFFU);
    Mcu_SmcSocSwmTimeoutWkupConfig(base, TRUE);

    /* configure soc wakeup smc timeout */
    Mcu_SmcSocWkupAckTimeoutConfig(base, 0xFFFFU);

    /* configure soc wakeup smc timeout */
    Mcu_SmcSocToRunTimeoutConfig(base, 0xFFFFU);

    /* configure soc lp2 wakeup timeout */
    Mcu_SmcSocLp2wkupTimeoutConfig(base, 0xFFFFU, TRUE);
}
