...
typedef struct __SBC_ErrorCode {
    uint8 SBC_Register;       //!< The register where an error occurred.
    uint8 flippedBitsMask;    //!< Masks the bits that differ from the expected value. Is 0 if readout is as expected.
    uint8 expectedValue;      //!< Expected readout of the register.
} SBC_ErrorCode;

/**
 * @brief   Typedef for interrupt callbacks.
 */
typedef void (*SBC_Func_Callback)(uint8 callbackHandler);
 ...
 *          The function must accept a uint8 as first argument.
 */
