...
static    uint32                          SBC_ISR_Vectors[30];
static    uint8                           SBC_ISR_ReadOutRegs[30];
static    uint8                           SBC_ISR_ReadOutVals[30];
static    uint8                           SBC_RegisteredCallbacks = 0;
static    uint8                           SBC_RegisteredRedoutRegs = 0;
...
SBC_ErrorCode SBC_WD_Trigger(void) {
    ...
    uint8 WD_Data = (uint8)SBC_Read_Command(SBC_WD_CTRL);
    return SBC_Write_Reg(SBC_WD_CTRL, WD_Data, NULL);
}


uint16 SBC_Read_Command(uint8 SBC_Reg) {
...
uint8 SBC_Read_RegField(uint8 SBC_Reg, uint8 SBC_FieldMsk, uint8 SBC_FieldPos) {
    uint8 data = ((uint8)SBC_Read_Command(SBC_Reg)) & 0xFFU;
    return (data & SBC_FieldMsk) >> SBC_FieldPos;
}


SBC_ErrorCode SBC_Write_Reg(uint8 SBC_Reg, uint8 SBC_Val, uint16 * returnval) {
    SBC_ErrorCode errCode;
    uint16 returndata = SBC_SPI_TRANSFER16(SBC_Write_Bit | SBC_Reg, SBC_Val);
    if(returnval != NULL) {
        *returnval = returndata;
    }

    errCode.SBC_Register = SBC_Reg;
    errCode.flippedBitsMask = ((uint8)SBC_Read_Command(SBC_Reg)) ^ SBC_Val;
    errCode.expectedValue = SBC_Val;
    return errCode;
}


SBC_ErrorCode SBC_Write_RegField(uint8 SBC_Reg, uint8 SBC_FieldMsk, uint8 SBC_FieldPos, uint8 SBC_FieldVal, uint16 * returnval) {
    SBC_ErrorCode errCode;

    /* Read data out of register to be manipulated */
    uint16 returndata = SBC_Read_Command(SBC_Reg);
    if(returnval != NULL) {
        *returnval = returndata;
    }
    uint8 data = (uint8)returndata;
    ...
    uint8 actualValue = ((uint8)SBC_Read_Command(SBC_Reg)) & SBC_FieldMsk;
...
SBC_ErrorCode SBC_Init(void) {
    SBC_ErrorCode errCode;
    uint8 WD_Checksum = CTRL_WD_CTRL;
    uint8 WD_CTRL = CTRL_WD_CTRL;
    uint8 SWK_ID3_CTRL, SWK_ID2_CTRL, SWK_ID1_CTRL, SWK_ID0_CTRL;
    uint8 SWK_MASK_ID3_CTRL, SWK_MASK_ID2_CTRL, SWK_MASK_ID1_CTRL, SWK_MASK_ID0_CTRL;
    uint8 SWK_CAN_FD_CTRL = CTRL_SWK_CAN_FD_CTRL;
    ...
    if((CTRL_SWK_ID0_CTRL & SBC_SWK_ID0_CTRL_IDE_Msk) == SBC_IDE_EXTENDED) {
        ...
        /* Standard length ID */
        uint16 SWK_ID_CTRL = CTRL_SWK_IDx_CTRL;
        uint32 SWK_MASK_ID_CTRL = CTRL_SWK_MASK_IDx_CTRL;
...
void SBC_Register_Callback(uint32 ISR_Vector, void (*Callback_Handler)(uint8 callbackHandler)) {
    /* Save callback */
    ...
    uint8 RegFound = 0;
    for (uint8 i = 0; i < SBC_RegisteredRedoutRegs; i++) {
    ...
    if (RegFound == 0) {
        SBC_ISR_ReadOutRegs[SBC_RegisteredRedoutRegs] = (uint8)(ISR_Vector >> 24);
...
SBC_ErrorCode SBC_ISR(void) {
    ...
    for (uint8 i = 0; i < SBC_RegisteredRedoutRegs; i++) {
        SBC_ISR_ReadOutVals[i] = (uint8) SBC_Read_Command(SBC_ISR_ReadOutRegs[i]);
    }

    /* Handle all interrupts */
    for (uint8 i = 0; i < SBC_RegisteredCallbacks; i++) {
        /* Decode ISR Vector */
        uint8 Compare = (uint8)SBC_ISR_Vectors[i];
        uint8 FieldPos = (uint8)(SBC_ISR_Vectors[i] >> 8);
        uint8 FieldMsk = (uint8)(SBC_ISR_Vectors[i] >> 16);
        uint8 RegAddr = (uint8)(SBC_ISR_Vectors[i] >> 24);

        /* Readback of associated status-bit */
        uint8 ReadBack = 0;
        for (uint8 j = 0; j < SBC_RegisteredRedoutRegs; j++) {
...
SBC_ErrorCode SBC_Mode_Sleep(void) {
    ...
    if(MATH_EN_PN != 0) {
        ...
        registerReadOut = (uint8)SBC_Read_Command(SBC_SWK_STAT);
    ...
    registerReadOut = (uint8)SBC_Read_Command(SBC_M_S_CTRL);
...
uint16 SBC_SYS_STAT_Read(void) {
    uint16 systemStatus = 0;
    uint16 returndata = SBC_Read_Command(SBC_SYS_STATUS_CTRL_0);
...
SBC_ErrorCode SBC_SYS_STAT_Write(uint16 SystemStatus) {
    SBC_ErrorCode errCode;

    /* Write lower bits */
    errCode = SBC_Write_Reg(SBC_SYS_STATUS_CTRL_0, (uint8)SystemStatus, NULL);
    if(errCode.flippedBitsMask > 0) {
        return errCode;
    }

    /* Write higher bits */
    return SBC_Write_Reg(SBC_SYS_STATUS_CTRL_1, (uint8)(SystemStatus >> 8), NULL);
}
