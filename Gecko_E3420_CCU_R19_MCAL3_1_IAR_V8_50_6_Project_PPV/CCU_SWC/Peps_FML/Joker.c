...
static uint8_t u8CmdBuff[255]; /**< \brief SPI�������������?*/
static uint8_t u8ResBuff[255]; /**< \brief SPI��Ӧ���������� */
...
stat_t JOKER_SetLfData(uint8_t u8DataIndex, uint8_t u8BytesLen, uint8_t *pu8DataBuff)
{
    ...
    i = 0x00;
    while (_JOKER_CmdTransmit(SET_LF_DATA_CMD, u8CmdBuff, u8ResBuff, 0x05 + u8BytesLen, 200) != 0)
    {
        i++;
        if (i == 3)
        {
...
stat_t JOKER_SetLfNrz(uint8_t u8DataIndex, uint8_t u8BytesLen, uint8_t *pu8DataBuff)
{
    ...
    i = 0x00;
    while (_JOKER_CmdTransmit(SET_LF_DATA_CMD, u8CmdBuff, u8ResBuff, 0x05 + u8BytesLen, 300) != 0)
    {
        i++;
        if (i == 3)
        {
...
stat_t JOKER_ConfigTimerPolling(void)
{
    ...
    i = 0x00;
    while (_JOKER_CmdTransmit(CONFIG_TIMER_POLLING_CMD, u8CmdBuff, u8ResBuff, 0x0C, 600) != 0)
    {
        i++;
        if (i == 3)
            return SF_SPI;
...
stat_t NJJ29C0_ConfigTimerPolling(uint8_t drpi, uint16_t ptime, uint8_t len, uint8_t *dataid)
{
    ...
    i = 0x00;
    while (_JOKER_CmdTransmit(CONFIG_TIMER_POLLING_CMD, u8CmdBuff, u8ResBuff, 9 + len, 600) != 0)
    {
        i++;
        if (i == 3)
        {
                                 ...
                                 uint8_t Ant3, uint8_t DataId_Len3, uint8_t *Lf_Ant3_DataIdx)
{
    ...
    if (Ant1 != DRP_NULL)
    {
        ...
        u8CmdBuff[txlen++] = DRP_NULL;    /**< \brief LCDRPi     : �͹���LF Driver */
        u8CmdBuff[txlen++] = 0x00;        /**< \brief RFU        : */
        u8CmdBuff[txlen++] = DataId_Len1; /**< \brief LENDATAIDi : ���ݶ��� */
    ...
    if (Ant2 != DRP_NULL)
    {
        ...
        u8CmdBuff[txlen++] = DRP_NULL;    /**< \brief LCDRPi     : �͹���LF Driver */
        u8CmdBuff[txlen++] = 0x00;        /**< \brief RFU        : */
        u8CmdBuff[txlen++] = DataId_Len2; /**< \brief LENDATAIDi : ���ݶ��� */
    ...
    if (Ant3 != DRP_NULL)
    {
        ...
        u8CmdBuff[txlen++] = DRP_NULL;    /**< \brief LCDRPi	   : �͹���LF Driver */
        u8CmdBuff[txlen++] = 0x00;        /**< \brief RFU 	   : */
        u8CmdBuff[txlen++] = DataId_Len3; /**< \brief LENDATAIDi : ���ݶ��� */
...
stat_t Peps_Fun_ConfigTimerPolling(uint8_t Cmd, uint8_t Incar_Ant, uint16_t ptime0, uint8_t LfDoor_Ant, uint16_t ptime1, uint8_t RfDoor_Ant, uint16_t ptime2)
{
    ...
    else if ((Cmd == 0x11) || (Cmd == 0x12))
    {
        ...
        u8CmdBuff[2] = 0x00;            /**< \brief LCDRPi     : �͹���LF Driver */
        u8CmdBuff[3] = 0x00;            /**< \brief LCDRNi     : */
        u8CmdBuff[4] = (uint8_t)ptime1; /**< \brief PTIME_Li   : pollingʱ�䣬��λΪ1ms */
        ...
        u8CmdBuff[14] = 0x00;            /**< \brief LCDRPi	   : �͹���LF Driver */
        u8CmdBuff[15] = 0x00;            /**< \brief LCDRNi	   : */
        u8CmdBuff[16] = (uint8_t)ptime0; /**< \brief PTIME_Li   : pollingʱ�䣬��λΪ1ms */
    ...
    i = 0x00;
    while (_JOKER_CmdTransmit(CONFIG_TIMER_POLLING_CMD, u8CmdBuff, u8ResBuff, txlen + 2, 600) != 0)
    {
        i++;
        if (i == 3)
        {
...
stat_t WelcomeGuest_ConfigTimerPollingOne(uint8_t Incar_Ant, uint16_t ptime0, uint8_t LfDoor_Ant, uint16_t ptime1, uint8_t RfDoor_Ant, uint16_t ptime2)
{
    ...
    u8CmdBuff[4] = (uint8_t)ptime0; /**< \brief PTIME_Li   : pollingʱ�䣬��λΪ1ms */
    u8CmdBuff[5] = (uint8_t)(ptime0 >> 8);
    u8CmdBuff[6] = 5; /**< \brief LENDATAIDi : 数杮段数 */
    ...
    u8CmdBuff[15] = 0x00;            /**< \brief LCDRNi	   : */
    u8CmdBuff[16] = (uint8_t)ptime1; /**< \brief PTIME_Li   : polling时间，坕佝为1ms */
    u8CmdBuff[17] = (uint8_t)(ptime1 >> 8);
    u8CmdBuff[18] = 1; /**< \brief LENDATAIDi : 数杮段数 */
    u8CmdBuff[19] = DATA_ID_DATA_RSSI;
    ...
    u8CmdBuff[23] = 0x00;            /**< \brief LCDRNi	   : */
    u8CmdBuff[24] = (uint8_t)ptime2; /**< \brief PTIME_Li   : polling时间，坕佝为1ms */
    u8CmdBuff[25] = (uint8_t)(ptime2 >> 8);
    u8CmdBuff[26] = 1; /**< \brief LENDATAIDi : 数杮段数 */
    u8CmdBuff[27] = DATA_ID_DATA_RSSI;

    i = 0x00;
    while (_JOKER_CmdTransmit(CONFIG_TIMER_POLLING_CMD, u8CmdBuff, u8ResBuff, 30, 600) != 0)
    {
        i++;
        if (i == 3)
        {
...
stat_t WelcomeGuest_ConfigTimerPollingTwo(uint8_t wakeid_num, uint8_t drpi0, uint16_t ptime0, uint8_t drpi1, uint16_t ptime1, uint8_t Carrier_OnOff)
{
    ...
    i = 0x00;
    while (_JOKER_CmdTransmit(CONFIG_TIMER_POLLING_CMD, u8CmdBuff, u8ResBuff, txlen, 600) != 0)
    {
        i++;
        if (i == 3)
        {
...
stat_t WelcomeGuest_ConfigTimerPollingWakeUp(uint8_t drpi, uint16_t ptime)
{
    ...
    u8CmdBuff[2] = 0x00;           /**< \brief LCDRPi	  : �͹���LF Driver */
    u8CmdBuff[3] = 0x00;           /**< \brief LCDRNi	  : */
    u8CmdBuff[4] = (uint8_t)ptime; /**< \brief PTIME_Li   : pollingʱ�䣬��λΪ1ms */
...
stat_t WelcomeGuest_Unlock_PollingWakeUp(uint8_t drpi0, uint8_t drpi1, uint16_t ptime)
{
    ...
    u8CmdBuff[3] = 0x00;  /**< \brief LCDRNi	  : */
    u8CmdBuff[4] = 4;     /**< \brief PTIME_Li   : pollingÊ±ï¿½ä£¬ï¿½ï¿½Î»Îª1ms */
    u8CmdBuff[5] = 0;
    ...
    if (drpi1 > 0)
    {
        ...
        u8CmdBuff[14] = 0x00;           /**< \brief LCDRNi	  : */
        u8CmdBuff[15] = (uint8_t)ptime; /**< \brief PTIME_Li   : pollingÊ±ï¿½ä£¬ï¿½ï¿½Î»Îª1ms */
        u8CmdBuff[16] = (uint8_t)(ptime >> 8);
        u8CmdBuff[17] = 4; /**< \brief LENDATAIDi : ï¿½ï¿½ï¿½Ý¶ï¿½ï¿½ï¿½ */
        u8CmdBuff[18] = DATA_ID_PREMABLE;
        u8CmdBuff[19] = DATA_ID_CV;
        u8CmdBuff[20] = DATA_ID_WUP1;
        u8CmdBuff[21] = DATA_ID_DATA_PE;
...
stat_t JOKER_StartTimerPolling(void)
{
    ...
    i = 0x00;
    while (_JOKER_CmdTransmit(START_TIMER_POLLING_CMD, u8CmdBuff, u8ResBuff, 0x02, 300) != 0)
    {
        i++;
        if (i == 3)
        {
...
void JOKER_Init(void)
{
    ...
    else
    {
        ...
        g_datCan1Tx_0x330[3] = u8NJJ29c0InitStep;
