...
#define Ea_DetReport(ApiId, ErrorId) ((void)Det_ReportError(EA_MODULE_ID, EA_INSTANCE_ID, (ApiId), (ErrorId)))
#endif

#define Ea_DetReportRuntime(ApiId, ErrorId) \
...
#include "Ea_MemMap.h"
static CONST(Ea_VoidFunctionVoidPtr, NVM_CONST) EA_InterServFunc[] = {
...
#endif /* #if (EA_SETMODE_API == STD_ON) */
...
#endif                          /* #if (EA_POLLING_MODE == STD_ON) */
        (*EA_InterServFunc[Ea_RunTimeData.EaRequestJob])();
    }
}
...
static FUNC(void, EA_PUBLIC_CODE) Ea_InterCopyReadData(void)
{
    ...
    uint8* src = &Ea_RunTimeData.EaPendReq.InterDataBufferPtr[EA_TOTAL_HEADER_LENGTH];
...
static FUNC(void, EA_PUBLIC_CODE) Ea_InterCheckReadHeader(void)
{
    ...
    uint8* src = Ea_RunTimeData.EaPendReq.InterDataBufferPtr;
    uint16 BlockNumber;
    uint16 BlockSize;
    uint32 BlockWriteCycle;
    uint16 BlockValid;
