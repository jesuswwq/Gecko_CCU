...
/* PRQA S 3432 -- */ /* MISRA Rule 20.7 */
{
    ...
    uint8 *daq_tmpptr = daqPtr->eventBufferPtr;
    /* traverse all odt */
    for (odtNum = 0u; odtNum < daqPtr->daqListSize; odtNum++)
    {
        ...
        for (entryNum = 0u; entryNum < daqPtr->odt[odtNum].XcpOdtEntryCount; entryNum++)
        {
            ...
            {
                if(daqPtr->eventBufferPtr == NULL)
                {
                    daqPtr->eventBufferPtr = daq_tmpptr;
                }
...
static FUNC(void, XCP_CODE) Xcp_AllocDaqHal(void)
{
    ...
    {
        for (daqNum = XCP_MIN_DAQ; daqNum < (XCP_MIN_DAQ + daqCount); daqNum++)
        {
